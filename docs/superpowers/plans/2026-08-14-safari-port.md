...
- **`world: "MAIN"` needs Safari 17.4+** (March 2024). `content/observe.js` (the platform-confirmed half of two-signal repost/share crediting) runs there. Resolution: set the app's minimum deployment targets to iOS 17.4 / macOS 14.4 rather than building a degraded mode — in August 2026 that floor excludes almost nobody and keeps the anti-fraud path identical on all three browsers.
