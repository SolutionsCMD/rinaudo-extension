...
(function () {
  ...
  // While a reply is being composed, report what the user clicks/keys so we can see why some
  // comments don't register — element testids + lengths ONLY, never the comment text.
  ...
  document.addEventListener('click', (e) => {
    ...
    dbg({ ev: 'click', onStatus: onStatusPage(), ref: onStatusPage() ? adapter.getRef() : cardRef(e.target),
          textLen: txt.length, clickedTestid: testid || null, submitMatched: !!adapter.commentSubmitTarget(e.target) });
  }, true);
  document.addEventListener('keydown', (e) => {
    if (e.key !== 'Enter') return;
    const txt = (adapter.commentText() || '').trim();
    if (!txt) return;
    dbg({ ev: 'key', onStatus: onStatusPage(), ref: onStatusPage() ? adapter.getRef() : cardRef(e.target),
          textLen: txt.length, meta: !!e.metaKey, ctrl: !!e.ctrlKey, shift: !!e.shiftKey });
  }, true);
